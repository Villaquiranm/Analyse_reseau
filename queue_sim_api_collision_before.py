        ...
        if(self.busy == False): 
            ...
            if (current_packet_drop == self.packets_drop):
                if (self.buffer_max_size is None or buffer_futur_size <= self.buffer_max_size):
                    ...
                    if self.debug:
                        print("Packet %d is discarded. Reason: Buffer overflow. Destination: %s" % (packet.id, self.name))
                    self.packets_drop += 1
            else:
                if self.debug:
...
if __name__=="__main__":
    ...
    qs1= QueuedServer(env,"Fil1", buffer_max_size=math.inf, service_rate=math.inf,debug=False)
    qs2= QueuedServer(env,"Fil2", buffer_max_size=math.inf, service_rate=math.inf,debug=False)
    ...
    env.run(until=500)
    #print(np.mean(qs3_monitor.sizes))
    print("Packets : %d, Dropped packets : %d" % (qs3.packet_count, qs3.packets_drop))
  #  print("%s => %s" % (self.name, self.destination.name))
