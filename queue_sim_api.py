...
class QueuedServer(object):
    ...
    def run(self):
        ...
        while True:
            ...
            if (self.destination is not None) and (current_packet_drop == self.packets_drop):
                self.destination.put(packet)
            else:
                if self.debug:
                    print("Packet %d is discarded. Reason: Collision. Destination: %s" % (packet.id, self.name))
                self.packets_drop += 1
            self.busy=False
    ...
    def put(self, packet):
        ...
        if (self.buffer_max_size is None or buffer_futur_size <= self.buffer_max_size) and (not self.busy):
...
if __name__=="__main__":
    ...
    src2= Source(env, "Source 2",gen_distribution=gen_dist,size_distribution=dist_size,debug=True)
    qs1= QueuedServer(env,"Fil1", buffer_max_size=math.inf, service_rate=math.inf,debug=True)
    qs2= QueuedServer(env,"Fil2", buffer_max_size=math.inf, service_rate=math.inf,debug=True)
    qs3= QueuedServer(env,"routeur1", buffer_max_size=math.inf, service_rate=process_rate,debug=True)
    ...
    env.run(until=100)
    #print(np.mean(qs1_monitor.sizes))
